...
def log_in(request, template_name='users/login_form.html'):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = authenticate(username=email, password=password) # this is a bug
            if user is not None:
                login(request, user)
                url = request.POST.get('source_url', '/')
                return redirect(url)
            else:
                context = {'form': form, 'error': "password or e-mail is not ture!"}
                return TemplateResponse(request, template_name, context)
...
def log_out(request, template_name='users/logout.html'):
    logout(request)
    context = {}
    return TemplateResponse(request, template_name, context)
